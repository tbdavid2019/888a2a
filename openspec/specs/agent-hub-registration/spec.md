...
The Hub SHALL support `closed`, `open`, and `public` modes. `closed` SHALL require existing tenant IAM, `open` SHALL require a bootstrap registration token, and `public` SHALL allow registration without a bootstrap token only when the operator explicitly enables it.
