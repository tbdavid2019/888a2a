# a2a-agent-network Specification

## Purpose
建立以 A2A 1.0 為正式協議的內部 Agent Network，使至少十幾個 888a2a Agent 能發現彼此、交換長任務、並行委派、回傳 Artifact 與在斷線後恢復。

## Requirements

### Requirement: Every enabled Agent has an A2A Agent Card
The system SHALL expose an A2A 1.0 Agent Card for every enabled Agent with its identity, interfaces, skills, media types, capabilities and security requirements.
