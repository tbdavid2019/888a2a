...
The system SHALL treat an Organization as the mandatory tenant boundary for every human, agent, service account, workspace, conversation, task, machine, connector, approval, audit event, credential, usage event, local filesystem object and S3-compatible object.
...
- **THEN** the resulting filesystem paths are distinct and each organization can access only its own path through the authenticated service
