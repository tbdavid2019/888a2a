...
Evidence notes (2026-08-27): Task 4.2 has an internal-only `WuKongIMAdapter` with private-host, redirect, tenant, cursor, and endpoint-boundary checks plus opt-in readiness and live send/sync gates. The controlled v2.2.5 image passed health and response-envelope checks, but repeated HTTP sends with the same `client_msg_no` returned different message IDs; ordering, durable deduplication, reconnect, offline-sync, failover, backup, and restore evidence must still be collected before checking 4.2.
...
- [x] 7.8 Add Provider install, update, broken and quarantined UI states; verify operators can roll back to the last verified runtime.

Evidence notes (2026-08-27): Tasks 7.1–7.7 are implemented by the manifest validator/registry, atomic runtime preparer, integrity metadata and quarantine path, pinned Claude adapter, per-Agent runtime isolation, launch fingerprints, and compatibility gating already present under `backend/agent/provider`, `backend/agent/runtime`, `backend/agent/executor`, and `backend/agent/client`. Task 7.8 now exposes provider failure details and real Prepare/Repair/Update actions through `RefreshMachineProvidersRequest.force_preparation` and the Machine control stream. Verified runtime history stores signed-by-digest manifest snapshots and binary hashes; `RefreshMachineProvidersRequest.rollback` selects the previous verified package after re-verifying its identity and disk contents. Runtime and Machine-profile tests cover rollback activation and repair after rollback.
...
Evidence notes (2026-08-28): Tasks 8.1–8.10 are covered by the official SDK boundary, tenant-scoped Agent Card/directory, durable work store/tools, orchestration graph/fan-out/join/cancellation, approval-required work state, deterministic 12-Agent acceptance topology, and the human-facing `/settings/a2a-graph` task graph/trace view. The production Manager now registers `/.well-known/agent-card.json` and `/a2a/v1/*` before the embedded SPA fallback, applies the normal bearer-token and organization-membership checks, and derives Agent Card URLs from the request host when no external URL is configured. A server regression test and remote VM check confirm the Agent Card is returned as JSON. The view exposes requester, delegate, status, artifacts, approvals, budget, failure cause, and collapsible descendants with frontend accessibility tests. Backend tests and `TestTwelveAgentAcceptanceGate` passed in GitHub Actions run `33027261306`.
...
- [x] 12.4 Perform security review for tenant isolation, webhook signatures, OAuth, secret storage, runtime sandbox, SSRF, file handling and approval binding; verify all critical/high findings are fixed or formally blocked from release.
...
Evidence notes (2026-08-27): Task 11.5 is covered by `backend/connector/fallback.go`, which requires explicit tenant/source/destination/event context for unsupported capability divergence and never reports simulated success. Task 12.1 now propagates bounded correlation IDs and tenant context through Connect handlers and structured audit responses, exports tenant-scoped operation counters and latency histograms from `/metrics`, and documents the contract in `docs/guide/observability-slo.md`; production metrics backend/Grafana dashboard deployment and an external end-to-end trace remain pending. Task 12.2 now has a transactional connector raw-payload redaction slice with legal holds and auditable outcomes; Message Plane, object storage, task, audit, and runtime retention coverage remains open. Task 12.3 now has `scripts/backup_888a2a.sh` plus `docs/guide/backup-restore.md`; the isolated PostgreSQL restore drill passed with checksum verification, core count parity, and a 36-second restore, recorded in `docs/evidence/backup-restore-2026-08-27.md`. WuKongIM failover, object-storage recovery, multi-node Manager recovery, and declared production RPO/RTO remain pending. Task 12.4 is covered by `docs/security/2026-08-27-security-review.md`; implemented critical/high paths have passing evidence, while missing external provider/HA work is explicitly blocked from release. Task 12.5 now has the opt-in `TestMessagePlaneHotChannelLoadGate` with configurable count, concurrency, and SLO target; the representative production load matrix remains pending. Task 12.6 is covered by the existing MessagePlane dual-projection, reconciliation, and per-Organization LEGACY/DUAL/MESSAGE_PLANE rollback gates. Task 12.8 is covered by the Docker, Agent Network, LINE, approval, A2A graph, and migration documentation indexes. Tasks 12.1, 12.3, 12.5, and 12.7 remain open for production observability, full DR/load evidence, and removal of compatibility paths.
