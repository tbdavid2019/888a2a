...
func frontendStaticSkipper(c *echo.Context) bool {
	...
	return strings.HasPrefix(p, "/v1") || strings.HasPrefix(p, "/machine/") || strings.HasPrefix(p, "/a2a/") || p == "/.well-known/agent-card.json" || p == "/metrics" || p == "/healthz" || p == "/api/version" || strings.HasPrefix(p, "/api/widget/") || strings.HasPrefix(p, "/assets/")
}
