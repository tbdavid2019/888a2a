...
-- Seed existing principals into default organization as OWNERs
...
-- Add organization_id / workspace_id columns to collaboration entities
...
ALTER TABLE user_channel_cursor ADD COLUMN IF NOT EXISTS device_id TEXT NOT NULL DEFAULT 'default';
ALTER TABLE agent_channel_cursor DROP CONSTRAINT IF EXISTS agent_channel_cursor_pkey;
ALTER TABLE agent_channel_cursor ADD CONSTRAINT agent_channel_cursor_pkey PRIMARY KEY (organization_id, agent_id, conversation_id);
ALTER TABLE user_channel_cursor DROP CONSTRAINT IF EXISTS user_channel_cursor_pkey;
ALTER TABLE user_channel_cursor ADD CONSTRAINT user_channel_cursor_pkey PRIMARY KEY (organization_id, principal_id, device_id, conversation_id);
CREATE INDEX IF NOT EXISTS idx_agent_channel_cursor_tenant_agent ON agent_channel_cursor(organization_id, agent_id, conversation_id);
CREATE INDEX IF NOT EXISTS idx_user_channel_cursor_tenant_device ON user_channel_cursor(organization_id, principal_id, device_id, conversation_id);
...
UPDATE command c SET organization_id = conv.organization_id FROM conversation conv WHERE conv.id = c.conversation_id;
UPDATE chat_message cm SET organization_id = conv.organization_id, workspace_id = conv.workspace_id FROM conversation conv WHERE conv.id = cm.conversation_id;
UPDATE conversation_member_meta cm SET organization_id = conv.organization_id FROM conversation conv WHERE conv.id = cm.conversation_id;
UPDATE command_conversation cc SET organization_id = conv.organization_id FROM conversation conv WHERE conv.id = cc.conversation_id;
UPDATE command_token_usage cu SET organization_id = a.organization_id FROM agent a WHERE a.id = cu.agent_id;
UPDATE agent_channel_cursor ac SET organization_id = conv.organization_id FROM conversation conv WHERE conv.id = ac.conversation_id;
UPDATE user_channel_cursor uc SET organization_id = conv.organization_id FROM conversation conv WHERE conv.id = uc.conversation_id;
UPDATE thread_participant tp SET organization_id = conv.organization_id FROM chat_message cm JOIN conversation conv ON conv.id = cm.conversation_id WHERE cm.id = tp.thread_root_message_id;
UPDATE message_reaction mr SET organization_id = cm.organization_id FROM chat_message cm WHERE cm.id = mr.message_id;
...
CREATE TABLE IF NOT EXISTS a2a888_message_projection (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (organization_id, message_id),
    CONSTRAINT a2a888_message_projection_message_fk FOREIGN KEY (organization_id, message_id) REFERENCES a2a888_message(organization_id, message_id) ON DELETE CASCADE,
    CONSTRAINT a2a888_message_projection_identity_check CHECK (organization_id <> '' AND conversation_id <> '' AND client_msg_no <> '' AND sender_id <> ''),
    CONSTRAINT a2a888_message_projection_sequence_check CHECK (message_seq > 0)
);
CREATE UNIQUE INDEX IF NOT EXISTS uq_a2a888_message_projection_sequence ON a2a888_message_projection(organization_id, conversation_id, message_seq);
CREATE INDEX IF NOT EXISTS idx_a2a888_message_projection_history ON a2a888_message_projection(organization_id, conversation_id, message_seq);
...
CREATE TABLE IF NOT EXISTS a2a888_message_projection_cursor (
    ...
    CONSTRAINT a2a888_message_projection_cursor_identity_check CHECK (organization_id <> '' AND conversation_id <> '' AND consumer_type <> '' AND consumer_id <> ''),
    CONSTRAINT a2a888_message_projection_cursor_sequence_check CHECK (message_seq >= 0)
);
CREATE INDEX IF NOT EXISTS idx_a2a888_message_projection_cursor_consumer ON a2a888_message_projection_cursor(organization_id, consumer_type, consumer_id);
...
CREATE TABLE IF NOT EXISTS a2a888_retention_outcome (
    ...
    resource_type TEXT NOT NULL, resource_id TEXT NOT NULL, action TEXT NOT NULL, detail TEXT NOT NULL DEFAULT '',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS a2a888_billing_account (
    ...
    currency TEXT NOT NULL DEFAULT 'USD', grace_policy TEXT NOT NULL DEFAULT 'READ_ONLY',
...
CREATE TABLE IF NOT EXISTS a2a888_subscription (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    name TEXT NOT NULL, state TEXT NOT NULL DEFAULT 'TRIAL', effective_from TIMESTAMPTZ NOT NULL,
    effective_until TIMESTAMPTZ, grace_policy TEXT NOT NULL DEFAULT 'READ_ONLY',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (organization_id, name), CONSTRAINT a2a888_subscription_identity_check CHECK (name <> ''),
...
CREATE INDEX IF NOT EXISTS idx_a2a888_subscription_active ON a2a888_subscription(organization_id, state, effective_from, effective_until);
CREATE TABLE IF NOT EXISTS a2a888_entitlement (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    feature TEXT NOT NULL, enabled BOOLEAN NOT NULL DEFAULT FALSE, quota_limit BIGINT NOT NULL DEFAULT 0,
    unit TEXT NOT NULL DEFAULT '', period TEXT NOT NULL DEFAULT '', overage_decision TEXT NOT NULL DEFAULT 'DENY',
    effective_from TIMESTAMPTZ NOT NULL DEFAULT now(), effective_until TIMESTAMPTZ,
    PRIMARY KEY (organization_id, feature), CONSTRAINT a2a888_entitlement_identity_check CHECK (feature <> ''),
...
CREATE TABLE IF NOT EXISTS a2a888_usage_event (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    name TEXT NOT NULL, workspace_id TEXT, principal_id TEXT NOT NULL DEFAULT '', agent_id TEXT NOT NULL DEFAULT '',
    feature TEXT NOT NULL, quantity BIGINT NOT NULL, unit TEXT NOT NULL, occurred_at TIMESTAMPTZ NOT NULL,
    source_reference TEXT NOT NULL DEFAULT '', idempotency_key TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (organization_id, name), CONSTRAINT a2a888_usage_event_workspace_fk FOREIGN KEY (organization_id, workspace_id) REFERENCES workspaces(organization_id, id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_a2a888_usage_event_idempotency ON a2a888_usage_event(organization_id, idempotency_key);
CREATE INDEX IF NOT EXISTS idx_a2a888_usage_event_period ON a2a888_usage_event(organization_id, feature, unit, occurred_at);
CREATE TABLE IF NOT EXISTS a2a888_usage_aggregate (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    feature TEXT NOT NULL, unit TEXT NOT NULL, period_start TIMESTAMPTZ NOT NULL, period_end TIMESTAMPTZ NOT NULL,
    quantity BIGINT NOT NULL DEFAULT 0, recomputed_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE OR REPLACE FUNCTION a2a888_reject_usage_event_update() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'usage events are immutable'; END $$;
DROP TRIGGER IF EXISTS a2a888_usage_event_immutable ON a2a888_usage_event;
CREATE TRIGGER a2a888_usage_event_immutable BEFORE UPDATE OR DELETE ON a2a888_usage_event FOR EACH ROW EXECUTE FUNCTION a2a888_reject_usage_event_update();

-- Durable entitlement and quota decisions
CREATE TABLE IF NOT EXISTS a2a888_quota_decision (
    id BIGSERIAL PRIMARY KEY, organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    feature TEXT NOT NULL, unit TEXT NOT NULL, requested_quantity BIGINT NOT NULL,
    consumed_quantity BIGINT NOT NULL, quota_limit BIGINT NOT NULL, decision TEXT NOT NULL,
    reason TEXT NOT NULL, period_start TIMESTAMPTZ NOT NULL, period_end TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS idx_a2a888_quota_decision_tenant ON a2a888_quota_decision(organization_id, feature, created_at DESC);

-- Tenant-scoped encrypted connector credentials
CREATE TABLE IF NOT EXISTS a2a888_connector_credential (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    installation_id TEXT NOT NULL, ciphertext BYTEA NOT NULL, key_version TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS a2a888_connector_identity_map (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    installation_id TEXT NOT NULL, external_identity_type TEXT NOT NULL, external_identity_id TEXT NOT NULL,
    principal_id TEXT NOT NULL, linked_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS a2a888_connector_conversation_map (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    installation_id TEXT NOT NULL, external_conversation_id TEXT NOT NULL, conversation_name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS a2a888_approval_policy_version (
    organization_id TEXT NOT NULL, policy_name TEXT NOT NULL, version TEXT NOT NULL,
    workspace_id TEXT, resource_pattern TEXT NOT NULL DEFAULT '', agent_id TEXT NOT NULL DEFAULT '',
    skill TEXT NOT NULL DEFAULT '', action_type TEXT NOT NULL DEFAULT '', destination_pattern TEXT NOT NULL DEFAULT '',
    requester_class TEXT NOT NULL DEFAULT '', risk_level SMALLINT NOT NULL DEFAULT 0,
    approver_principal_ids TEXT[] NOT NULL DEFAULT '{}', approver_group_ids TEXT[] NOT NULL DEFAULT '{}',
    approver_roles TEXT[] NOT NULL DEFAULT '{}', required_approvals INTEGER NOT NULL, timeout_seconds INTEGER NOT NULL,
    on_timeout TEXT NOT NULL DEFAULT 'DENY', escalation_principal_ids TEXT[] NOT NULL DEFAULT '{}',
    escalation_group_ids TEXT[] NOT NULL DEFAULT '{}', escalation_roles TEXT[] NOT NULL DEFAULT '{}',
    prohibit_requester_approval BOOLEAN NOT NULL DEFAULT FALSE, prohibit_agent_owner_sole_approval BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY (organization_id, policy_name, version),
    CONSTRAINT a2a888_approval_policy_version_policy_fk FOREIGN KEY (organization_id, policy_name)
        REFERENCES a2a888_approval_policy(organization_id, name) ON DELETE CASCADE,
    CONSTRAINT a2a888_approval_policy_version_workspace_fk FOREIGN KEY (organization_id, workspace_id)
...
CREATE TABLE IF NOT EXISTS a2a888_approval_request (
    organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE, name TEXT NOT NULL,
    workspace_id TEXT, policy_name TEXT NOT NULL, policy_version TEXT NOT NULL,
    requester_principal_id TEXT NOT NULL, executing_agent_id TEXT NOT NULL DEFAULT '', executing_agent_owner_id TEXT NOT NULL DEFAULT '',
    action_json JSONB NOT NULL, intent_hash TEXT NOT NULL, state TEXT NOT NULL DEFAULT 'PENDING',
    required_approvals INTEGER NOT NULL, approval_count INTEGER NOT NULL DEFAULT 0, execution_nonce TEXT NOT NULL,
    expires_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    completed_at TIMESTAMPTZ, terminal_reason TEXT NOT NULL DEFAULT '', PRIMARY KEY (organization_id, name),
    CONSTRAINT a2a888_approval_request_policy_fk FOREIGN KEY (organization_id, policy_name, policy_version)
        REFERENCES a2a888_approval_policy_version(organization_id, policy_name, version) ON DELETE RESTRICT,
    CONSTRAINT a2a888_approval_request_workspace_fk FOREIGN KEY (organization_id, workspace_id)
        REFERENCES workspaces(organization_id, id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_a2a888_approval_request_nonce ON a2a888_approval_request (organization_id, execution_nonce);
CREATE INDEX IF NOT EXISTS idx_a2a888_approval_request_pending ON a2a888_approval_request (organization_id, state, expires_at);
CREATE TABLE IF NOT EXISTS a2a888_approval_decision (
    organization_id TEXT NOT NULL, name TEXT NOT NULL, request_name TEXT NOT NULL,
    approver_principal_id TEXT NOT NULL, approver_role TEXT NOT NULL DEFAULT '', outcome TEXT NOT NULL,
    reason TEXT NOT NULL DEFAULT '', intent_hash TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (organization_id, name),
    CONSTRAINT a2a888_approval_decision_request_fk FOREIGN KEY (organization_id, request_name)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_a2a888_approval_decision_approver ON a2a888_approval_decision (organization_id, request_name, approver_principal_id);
CREATE INDEX IF NOT EXISTS idx_a2a888_approval_decision_request ON a2a888_approval_decision (organization_id, request_name, created_at);
CREATE OR REPLACE FUNCTION a2a888_reject_approval_request_intent_update() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.organization_id <> OLD.organization_id OR NEW.name <> OLD.name OR NEW.policy_name <> OLD.policy_name OR NEW.policy_version <> OLD.policy_version
       OR NEW.requester_principal_id <> OLD.requester_principal_id OR NEW.executing_agent_id <> OLD.executing_agent_id
       OR NEW.executing_agent_owner_id <> OLD.executing_agent_owner_id OR NEW.action_json <> OLD.action_json
       OR NEW.intent_hash <> OLD.intent_hash OR NEW.execution_nonce <> OLD.execution_nonce OR NEW.expires_at <> OLD.expires_at THEN
...
CREATE TRIGGER a2a888_approval_request_intent_immutable BEFORE UPDATE ON a2a888_approval_request FOR EACH ROW EXECUTE FUNCTION a2a888_reject_approval_request_intent_update();
CREATE OR REPLACE FUNCTION a2a888_reject_approval_decision_update() RETURNS trigger LANGUAGE plpgsql AS $$
...
CREATE TRIGGER a2a888_approval_decision_immutable BEFORE UPDATE ON a2a888_approval_decision FOR EACH ROW EXECUTE FUNCTION a2a888_reject_approval_decision_update();
