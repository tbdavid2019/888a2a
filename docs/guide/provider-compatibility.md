...
| OpenClaw | Private Gateway `/v1/responses` | Loopback Gateway health, `/v1/models` auth, and `/v1/responses` turn gate passed on 2026-08-28. The configured Gateway exhausted its primary model quota and used its own fallback; this proves the bridge transport, not the primary provider model. | `A2A888_RUN_OPENCLAW_BRIDGE_TESTS=1 A2A888_OPENCLAW_GATEWAY_URL=http://127.0.0.1:18789 A2A888_OPENCLAW_GATEWAY_TOKEN=... go test ./backend/a2a -run TestOpenClawBridgeLiveGateIsOptIn -count=1` |
