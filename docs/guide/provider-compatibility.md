...
| agy / Antigravity | Bounded CLI | Deterministic bounded CLI tests pass. Live evidence is pending an authenticated local `agy` profile. | `A2A888_RUN_AGY_BRIDGE_TESTS=1 go test ./backend/a2a -run TestCommandBridgeRealRuntimeGatesAreOptIn/agy -count=1` |
